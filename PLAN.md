...
2. [ ] Implement LangGraph agent as MCP client (stdio subprocess), one persona hardcoded, structured output via Pydantic + validation-retry-once on parse failure
   - [x] MCP client path, Pydantic response model, one validation-correction retry, and Gemini/Mistral provider wiring are implemented; LangGraph orchestration remains.
